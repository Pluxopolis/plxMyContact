...
# Check IE version
function label_if_needed($label) {
	if(preg_match('/(?i)msie [1-8]/',$_SERVER['HTTP_USER_AGENT']) AND !preg_match('/msie 10./i', $_SERVER['HTTP_USER_AGENT'])) {
		echo $label;	
	}
}
...
<div id='form_contact'>
	...
	<form action='#form_contact' method='post'>
		<fieldset>
		<?php label_if_needed("<p><label for='name'><?php $plxPlugin->lang('L_FORM_NAME') ?>&nbsp;:</label></p>"); ?>
		...
		<?php label_if_needed("<p><label for='mail'><?php $plxPlugin->lang('L_FORM_MAIL') ?>&nbsp;:</label></p>"); ?>
		...
		<?php label_if_needed("<p><label for='message'><?php $plxPlugin->lang('L_FORM_CONTENT') ?>&nbsp;:</label></p>"); ?>
		...
		<?php if($captcha): ?>
		<?php label_if_needed("<p><label for='id_rep'><strong><?php $plxPlugin->lang('L_FORM_ANTISPAM') ?></strong>&nbsp;:</label></p>"); ?>
